...
class SslServerCustomizer implements JettyServerCustomizer {
	...
	private final List<JettyExProperties.Protocol> protocols;
	...
	SslServerCustomizer(List<JettyExProperties.Protocol> protocols, InetSocketAddress address, ClientAuth clientAuth, SslBundle sslBundle) {
	...
	private ServerConnector createServerConnector(Server server, SslContextFactory.Server sslContextFactory,
		...
		if (!this.protocols.contains(JettyExProperties.Protocol.HTTP2)) {
